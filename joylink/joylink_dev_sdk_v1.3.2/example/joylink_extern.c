...
joylink_dev_set_attr_jlp(JLPInfo_t *jlp)
{
     ...
     *FIXME:must to do
     */
    _g_pam->feedid.set = DevFeedidSet;  //add by wangshi
...
joylink_dev_get_snap_shot(char *out_snap, int32_t out_max)
{
     ...
     */
    int len = 0;
    joylink_get_wifi_attr(_g_pwifi);  //解析出频段、速度信息
...
joylink_dev_get_json_snap_shot(char *out_snap, int32_t out_max, int code, char *feedid)
{
	...
	memcpy(out_snap,DevJson.buff,DevJson.offset);
    sprintf(out_snap + DevJson.offset, "{\"code\":%d, \"feedid\":\"%s\"}", code, feedid);
...
joylink_dev_ota_status_upload()
{
    ...
    for(i = 0; i <= 100; i++){
        ...
        if(i > 100){
            ...
            strcpy(otaUpload.status_desc, "鍥轰欢瀹夎涓�");
        }else{
            otaUpload.status = 0;
            otaUpload.progress = i;
            strcpy(otaUpload.status_desc, "鍥轰欢涓嬭浇涓�");
        }      
    ...
    strcpy(otaUpload.status_desc, "鍥轰欢鍗囩骇瀹屾垚");
    joylink_server_ota_status_upload_req(&otaUpload);
}
