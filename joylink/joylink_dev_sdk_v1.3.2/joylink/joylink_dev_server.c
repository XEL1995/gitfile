...
joylink_proc_server_st()
{
    ...
    joylink_server_st_time_out_check();//判断网络连接失败次数，设置网络状态，若超时则重联

	switch (_g_pdev->server_st){  //根据网络连接模式，设置测试区间
        case JL_SERVER_ST_INIT:
            joylink_server_st_init();  //网络模块初始化，2s超时时间
            interval = 2000;
            break;
        case JL_SERVER_ST_AUTH:
            joylink_server_st_auth();  //发送accesskey
