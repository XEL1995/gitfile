...
//把一个过长的包分为长1024的分包，再给每一个加上header段和payload段，分别发出
{
    int ret = -1;
    int len;
    int i;
...
joylink_proc_lan_scan(uint8_t *src, struct sockaddr_in *sin_recv, socklen_t addrlen)
{
    ...
    if(E_RET_OK != joylink_parse_scan(&scan, (const char *)src)){  //解析出uuid和扫描状态
    }else{
        len = joylink_packet_lan_scan_rsp(&scan);  //创建发送数据包
    }
...
joylink_proc_lan_write_key(uint8_t *src, struct sockaddr_in *sin_recv, socklen_t addrlen)
{
    ...
    joylink_parse_lan_write_key(&de, (const char*)src + 4);  //解析服务器发来数据
    ...
    joylink_util_cut_ip_port(de.joylink_server,  //分解服务器ip port
            _g_pdev->jlp.joylink_server,
            &_g_pdev->jlp.server_port);

    joylink_dev_set_attr_jlp(&_g_pdev->jlp);  //存储信息

    int len = joylink_packet_lan_write_key_rsp();  //创建回应信息
    if(len > 0 && len < JL_MAX_PACKET_LEN){ 
...
//解析控制指令
{
    ...
    joylink_parse_json_ctrl(feedid, (char*)json_cmd);  //解析出feedid
    ret = joylink_dev_lan_json_ctrl((char *)json_cmd);  //解析出json格式控制命令并作用空调端

    memcpy(data, &tt, 4);
    //获取json格式的设备快照，要封入设备控制结果、feedid
    ...
    joylink_server_upload_req();  //设备状态上传到服务器
} 
...
joylink_proc_lan()
{
    ...
    joylink_util_get_ipstr(&sin_recv, _g_pdev->jlp.ip);  //服务端ip、端口存入全局变量

    ret = joylink_dencypt_lan_req(&param, recBuffer, ret,
    		                   recPainBuffer, JL_MAX_PACKET_LEN);
                               //获取数据type,解析出payload,返回payload长度
    if (ret <= 0){
        goto RET;
    }

    pHead = (JLPacketHead_t*)recBuffer;
    if(pHead->total > 1){  //判断是否需要分包
        log_info("PACKET WAIT TO JOIN!!!!!!:IP:%s", _g_pdev->jlp.ip);
        if(E_RET_ALL_DATA_HIT ==  joylink_join_pkg_add_data(_g_pdev->jlp.ip,   //将各个包的数据加入到链表
